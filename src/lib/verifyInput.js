...
const groupNameRegex = /^[ A-Za-z\d\s~!_@$%^()=\[\]{}|'";:\\?<>.,\/#&+-]{1,20}$/
const descriptionRegex = /^[ A-Za-z\d\s~!_@$%^*()=\[\]{}|'";:\\?<>.,\/#&+-]{2,144}$/

function verifyTax(tax){
    let errors = []
    if(!taxTypeRegex.test(tax.type)) errors.push('tax name must have more than 1 and not more than 20 characters of alphabets and numbers only')
...
function verifyUnit(unit){
    let errors = []
    if(!unitRegex.test(unit.unit)) errors.push('unit must have more than 1 and not more than 12 characters of alphabets and numbers only and must not contain spaces')
